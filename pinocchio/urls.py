...
urlpatterns = [
                  ...
                  url(r'^maintainTeam/(?P<roundPk>[0-9]+)/?$', views.maintain_team, name='maintainTeamR'),
                  ...
                  url(r'^accountDetails/(?P<userId>[0-9]+)$', views.account_details, name='accountDetails'),

                  url(r'^$', views.index, name='index'),
                  url(r'^(?P<question_id>[0-9]+)/$', views.detail, name='detail'),
                  url(r'^userAdmin/submitForm/?$', views.submit_form),
                  url(r'^userAdmin/submitCSV/$', views.submit_csv, name="submitCSV"),
                  url(r'^userAdmin/userProfile/(?P<userPk>[0-9]+)/?$', views.user_profile, name="userProfile"),
                  url(r'^userAdmin/delete/$', views.user_delete, name="userDelete"),
                  url(r'^userAdmin/update/(?P<userPk>[0-9]+)/?$', views.user_update),
                  url(r'^userAdmin/resetPassword/(?P<userPk>[0-9]+)/?$', views.reset_password),
                  ...
                  url(r'^maintainRound/delete/(?P<roundPk>[0-9]+)/?$', views.round_delete),
                  url(r'^maintainRound/update/(?P<roundPk>[0-9]+)/?$', views.round_update),
                  url(r'^maintainTeam/getTeamsForRound/(?P<roundPk>[0-9]+)/?$', views.get_teams_for_round),
                  url(r'^maintainTeam/getQuestionnaireForRound/(?P<roundPk>[0-9]+)/?$', views.get_questionnaire_for_round),
                  url(
                  ...
                  url(r'^report/getUser/(?P<userPk>[0-9]+)/?$', views.get_user),
